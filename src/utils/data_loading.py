...
CAR_LABEL = 2
BANANACAR_LABEL = 3
ALL_BANANA_CAR_LABELS = [BANANA_LABEL, CAR_LABEL, BANANACAR_LABEL]
...
def bc_loader(dir, name, label, batch_size=256, num_workers=0):
    ...
    dataset = BananaCarImageDataset(dir, name, label)
...
def all_bc_loader(dirs, names, labels, batch_size=256, num_workers=0):
    ...
    for i in range(len(dirs)):
        ...
        d = BananaCarImageDataset(dirs[i][0], name, label)
        datasets.append(d)
    ...
    for i in range(len(dirs)):
        ...
        d = BananaCarImageDataset(dirs[i][1], name, label)
        datasets.append(d)
    dataset = ConcatDataset(datasets)
    labels = [i[1] for i in dataset]
    ...
    for i in range(len(dirs)):
        ...
        d = BananaCarImageDataset(dirs[i][2], name, label)
        datasets.append(d)
    dataset = ConcatDataset(datasets)
    labels = [i[1] for i in dataset]
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            for i in idx:
                ...
                label = one_hot(label, ALL_BANANA_CAR_LABELS).view((len(ALL_BANANA_CAR_LABELS)))
